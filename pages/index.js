        ...
        <div>
          ...
          <button onClick={() => {
            ...
            const rows = active.map(s => {
              ...
              return '<div style="background:#1e293b;border-radius:4mm;overflow:hidden;border:1px solid #334155;display:flex;flex-direction:column"><div style="width:100%;aspect-ratio:1;overflow:hidden;background:#fff;display:flex;align-items:center;justify-content:center">' + img + '</div><div style="padding:4mm;display:flex;flex-direction:column"><div style="font-size:' + nameSz + 'pt;font-weight:800;color:#fff;line-height:1.2;margin-bottom:2mm;min-height:' + (nameSz * 2.8).toFixed(0) + 'pt">' + s.name + '</div>' + desc + '<div style="font-size:' + priceSz + 'pt;font-weight:900;color:#c8a84b">' + price + '</div></div></div>'
            }).join("")
            const css = '@page{size:A4 portrait;margin:0}*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;width:210mm;height:297mm;overflow:hidden;background:#0f172a}.page{width:210mm;height:297mm;display:flex;flex-direction:column}.hdr{background:#1e3a5f;padding:12mm 14mm 8mm;text-align:center;border-bottom:3px solid #c8a84b}.grid{flex:1;display:grid;grid-template-columns:' + cols + ';gap:6mm;padding:8mm;align-content:center}.ftr{background:#1e3a5f;padding:5mm 14mm;text-align:center;border-top:2px solid #c8a84b}'
            ...
            const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Specials</title><style>' + css + '</style></head><body><div class="page">' + hdrHtml + '<div class="grid">' + rows + '</div>' + ftrHtml + '</div></body></html>'
            const w = window.open('', '_blank')
            w.document.write(html)
            w.document.close()
