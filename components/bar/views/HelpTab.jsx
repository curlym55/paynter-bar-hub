      ...
      content: (
        <div>
          {[
            ...
            ['📊 Avg Buy Report', "Click 📊 Avg Buy Report (Stock Items → Pricing) to download an Excel export of the average buy price per item from actual supplier invoices — both a Recent (90-day) figure and a 365-Day Avg, side by side, plus your current Hub buy price and the difference against the 365-day figure. Recent (90-day) reacts faster to a genuine price change; 365-Day Avg smooths out one-off bulk orders. This is a reference report only — it does not change any Hub prices. Use it at the January/July review to spot items whose buy price has drifted."],
          ].map(([q, a], i, arr) => (
            <div key={q} style={{ display: 'flex', gap: 12, padding: '8px 0', borderBottom: i < arr.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
              <div style={{ width: 160, minWidth: 160, fontSize: 12, fontWeight: 600, color: '#374151' }}>{q}</div>
              <div style={{ fontSize: 12, color: '#4b5563', lineHeight: 1.65 }}>{a}</div>
