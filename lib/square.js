...
export async function fetchSquareData(token, daysBack = 90, kvGet = null, kvSet = null) {
  ...
  // Glass sales from Orders API -- cached separately to avoid slowing every refresh
  // Cache key includes daysBack so different periods stored independently
  let ordersSales = {}
  const glassCacheKey = `glassSales_${daysBack}`
  if (kvGet) {
    try { ordersSales = (await kvGet(glassCacheKey)) || {} } catch {}
  }
  if (!ordersSales || Object.keys(ordersSales).length === 0) {
    const periodStart = new Date()
    periodStart.setDate(periodStart.getDate() - daysBack)
    ordersSales = await fetchSalesReport(token, periodStart.toISOString(), new Date().toISOString())
    if (kvSet && Object.keys(ordersSales).length > 0) {
      kvSet(glassCacheKey, ordersSales, 6 * 60 * 60).catch(() => {})
    }
  }
