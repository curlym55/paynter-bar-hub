...
export async function fetchSquareData(token, daysBack = 90) {
  ...
  for (const [name, vars] of Object.entries(itemGroups)) {
    ...
    // For wines: include glass variation sales converted to bottle equivalents (150ml glass / 750ml bottle = 0.2 per glass)
    // For spirits and others: use Regular variation inventory changes only
    const glassVars = vars.filter(v => v.variationName.toLowerCase().includes('glass'))
    const hasGlass  = glassVars.length > 0
    const GLASS_ML  = 150
    const BOTTLE_ML = 750
    const glassFraction = GLASS_ML / BOTTLE_ML  // 0.2 bottles per glass sold

    const soldRegular = countVars.reduce((s, v) => s + (sold[v.varId] || 0), 0)
    const soldGlass   = hasGlass ? glassVars.reduce((s, v) => s + (sold[v.varId] || 0), 0) : 0
    const totalSoldBottles = hasGlass
      ? soldRegular + (soldGlass * glassFraction)  // combine bottle + glass sales
      : soldRegular
    const totalSold = Math.round(totalSoldBottles)
    const weeklyAvg = +(totalSoldBottles / (daysBack / 7)).toFixed(1)
