...
import { defaultCategory } from './calculations'
...
// Exported (in addition to being used internally by fetchSquareData) so
// callers that only need the current catalog — not stock levels or sales —
// can get it cheaply. See pages/api/invoices/avg-prices.js: it needs to know
// which items are still actually sold, without the cost of pulling inventory
// and sales data it doesn't use.
export async function getBarCategoryIds(token) {
...
export async function getCatalogItems(token, barCategoryIds) {
  const allItems = {}
  const variations = {}
  let cursor = null
  ...
  do {
    ...
    for (const order of orders) {
      for (const lineItem of order.line_items || []) {
        ...
        // For items with NO glass variation at all, 'Regular' is a whole
        // packaged unit (can, bottle, piccolo, snack packet) rather than a
        // pour -- EXCEPT Spirits/Fortified & Liqueurs, where a plain
        // 'Regular' sale is a nip pour, not a bottle (bottles of spirits
        // aren't retailed across the bar). Category is inferred from the
        // item name since fetchSalesReport doesn't have per-item settings
        // overrides available here.
        const category = defaultCategory(name)
        const isPourOnlyCategory = category === 'Spirits' || category === 'Fortified & Liqueurs'
        const isBottleSale = isBottle || (isRegular && (itemHasGlass[name] || !isPourOnlyCategory))
...
export async function postSingleWasteAdjustment(token, locationId, adj) {
  const body = {
    // Callers should pass a stable key (see wastage-sync.js): the same key AND
    // the same request body on a retry is what lets Square recognise a repeat
    // and NOT deduct the stock a second time. The Date.now() fallback gives
    // no such protection — it's only for callers that don't supply one.
    idempotency_key: adj.idempotencyKey || `waste-${adj.entryId}-${Date.now()}`,
    changes: [{
      type: 'ADJUSTMENT',
      adjustment: {
