...
export async function getVariationIdMap(token) {
  ...
  for (const [name, vars] of Object.entries(groups)) {
    ...
    const chosen = regular || vars[0]
    primaryVarIds[name] = chosen.varId
    console.log(`[varMap] "${name}": vars=[${vars.map(v => `"${v.variationName}"(${v.varId.slice(-6)})`).join(', ')}] → chose "${chosen.variationName}"(${chosen.varId.slice(-6)})`)
  }
