...
async function getCatalogItems(token, barCategoryIds) {
  ...
  do {
    ...
    for (const obj of data.objects || []) {
      ...
      if (obj.is_deleted) continue // is_archived check removed - some active items incorrectly flagged
...
export async function fetchSquareData(token, daysBack = 90, kvGet = null, kvSet = null) {
  ...
  if (!glassFromCache && kvSet) {
    // Background fetch — don't await, returns immediately
    const periodStart = nowAEST()
    periodStart.setDate(periodStart.getDate() - daysBack)
    fetchSalesReport(token, periodStart.toISOString(), nowAEST().toISOString())
      .then(sales => {
        if (Object.keys(sales).length > 0) {
          kvSet(glassCacheKey, sales, 6 * 60 * 60).catch(() => {})
        }
      })
      .catch(() => {})
    // ordersSales stays {} — wines will use inventory-only avg this time
  }
