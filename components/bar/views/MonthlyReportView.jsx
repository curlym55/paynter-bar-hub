...
export default function MonthlyReportView() {
  ...
  async function exportExcel() {
    ...
    rows.push([cell('  — spirits (per nip)', {font:{sz:9,color:{rgb:GREY}}}), cell(report.pricing.avgSpiritMarkupPct == null ? '—' : `${report.pricing.avgSpiritMarkupPct}%`, {font:{sz:9,color:{rgb:GREY}}}), cell(`${report.pricing.spiritsPriced} spirits`, {font:{sz:9,color:{rgb:GREY}}}), cell(''), cell('')])
    rows.push([cell('  — beer / wine / other', {font:{sz:9,color:{rgb:GREY}}}), cell(report.pricing.avgNonSpiritMarkupPct == null ? '—' : `${report.pricing.avgNonSpiritMarkupPct}%`, {font:{sz:9,color:{rgb:GREY}}}), cell(''), cell(''), cell('')])
    rows.push([])
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(160px, 1fr))', gap:10 }}>
            {[
              ...
              { label:'Avg markup', value: report.pricing.avgMarkupPct == null ? '—' : `${report.pricing.avgMarkupPct}%`, color:'#7c3aed',
                sub: `${report.pricing.itemsPriced} items priced${report.pricing.itemsSkipped ? ` · ${report.pricing.itemsSkipped} unpriced` : ''}` },
            ].map(s => (
