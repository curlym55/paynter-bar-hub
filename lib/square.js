...
export async function fetchSquareData(token, daysBack = 90) {
  ...
  // Also fetch Orders API sales for the same period — needed for wine glass sales
  // which don't generate inventory change events (glass variation not inventory-tracked)
  const periodStart = new Date()
  periodStart.setDate(periodStart.getDate() - daysBack)
  const ordersSales = await fetchSalesReport(token, periodStart.toISOString(), new Date().toISOString())
