...
export async function fetchSquareData(token, daysBack = 90, kvGet = null, kvSet = null) {
  ...
  for (const [name, vars] of Object.entries(itemGroups)) {
    ...
    // ── DEBUG: log inventory data for affected wines ──────────────────────
    const DEBUG_WINES = ['balliamo','marlborough sounds','rosemount chardonnay',
      "devil's corner",'mockingbird','pepperjack','wyndham','arrogant frog']
    if (DEBUG_WINES.some(w => name.toLowerCase().includes(w))) {
      console.log('[square-debug]', name)
      console.log('  itemId:', itemId)
      console.log('  vars:', vars.map(v => ({ id: v.varId, name: v.variationName })))
      console.log('  countVars:', countVars.map(v => ({ id: v.varId, name: v.variationName, inv: inventory[v.varId] })))
      console.log('  itemOnHand (item-level):', inventory[itemId])
      console.log('  varOnHand:', varOnHand, '| itemOnHand:', itemOnHand, '| final onHand:', onHand)
      // Log ALL inventory keys that contain anything for this item's IDs
      const allIds = [...vars.map(v => v.varId), itemId].filter(Boolean)
      const found = Object.entries(inventory).filter(([k]) => allIds.includes(k))
      console.log('  inventory hits:', found)
    }
    // ── END DEBUG ─────────────────────────────────────────────────────────
